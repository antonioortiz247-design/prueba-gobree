...
module.exports = async (req, res) => {
  if (req.method === 'GET') {
    ...
    try {
      const raw = await storageGet('projectsCards');
      if (raw) {
  ...
  if (req.method === 'POST') {
    ...
    const cleaned = projects
      .map((img) => ({
        category: String(img.category || '').trim().slice(0, 60),
        ...
        mediaAlt: String(img.mediaAlt || 'Proyecto Gobree').trim().slice(0, 200)
      }))
      .filter((img) => img.title && img.mediaSrc);

    if (!cleaned.length) {
      res.statusCode = 400;
      res.setHeader('Content-Type', 'application/json');
      res.end(JSON.stringify({ ok: false, error: 'empty_projects' }));
      return;
    }

    try {
      await storageSet('projectsCards', JSON.stringify(cleaned));
    } catch (e) {
