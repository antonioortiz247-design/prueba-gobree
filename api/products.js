...
const DEFAULT_PRODUCTS = require('../catalogo');

function cleanProduct(product, index) {
  const rawId = Number(product && product.id);
  const fallbackId = Date.now() + index;
  const tags = Array.isArray(product && product.tags)
    ? product.tags
    : String((product && product.tags) || '')
        .split(',')
        .map((tag) => tag.trim())
        .filter(Boolean);

  return {
    id: Number.isFinite(rawId) && rawId > 0 ? rawId : fallbackId,
    nombre: String((product && product.nombre) || '').trim().slice(0, 160),
    categoria: String((product && product.categoria) || 'Otros Productos').trim().slice(0, 120),
    descripcion: String((product && product.descripcion) || '').trim().slice(0, 500),
    tags: tags.map((tag) => String(tag || '').trim().slice(0, 60)).filter(Boolean).slice(0, 12),
    material: String((product && product.material) || '').trim().slice(0, 120),
    propiedad: String((product && product.propiedad) || '').trim().slice(0, 120),
    img: String((product && product.img) || '').trim().slice(0, 1_200_000),
    alt: String((product && product.alt) || (product && product.nombre) || 'Producto Gobree').trim().slice(0, 200),
    title: String((product && product.title) || (product && product.nombre) || 'Producto Gobree').trim().slice(0, 160)
  };
}
...
function cleanProducts(products) {
  return (Array.isArray(products) ? products : [])
    .map(cleanProduct)
    .filter((product) => product.nombre && product.categoria && product.descripcion);
}
...
module.exports = async (req, res) => {
  if (req.method === 'GET') {
    ...
    try {
      const raw = await storageGet('catalogProducts');
      if (raw) {
    ...
    res.end(JSON.stringify({ products: cleanProducts(products) }));
    return;
  }
  ...
  if (req.method === 'POST') {
    ...
    const cleaned = cleanProducts(data.products);
    if (!cleaned.length) {
      res.statusCode = 400;
      res.setHeader('Content-Type', 'application/json');
      res.end(JSON.stringify({ ok: false, error: 'empty_products' }));
      return;
    }

    try {
      await storageSet('catalogProducts', JSON.stringify(cleaned));
    } catch (e) {
